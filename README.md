...
To get an array of webhook definition objects, just require or import the module.
...
To get a JSON schema of webhook payloads you need to require it specifically:

```js
// Use Node.js require:
const SCHEMA = require("@octokit/webhooks-definitions/index.schema.json");

// Or ESM/TypeScript import:
import SCHEMA from "@octokit/webhooks-definitions/index.schema.json";
```
...
`node bin/octokit-webhooks --usage` for instructions. The [`index.schema.json`](index.schema.json) is a JSON Schema that you can use to validate webhook payloads.
