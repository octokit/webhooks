...
async function createPrOnChange () {
  ...
  // count changes
  const diffResult = await execa.shell('git diff --stat index.js')
  ...
  await execa('git', ['add', 'index.json', 'index.js'])
  await execa('git', ['commit', '-m', 'build: webhooks'])
  await execa('git', ['push', `https://${process.env.GH_TOKEN}@github.com/${owner}/${repo}.git`, `HEAD:${branchName}`])
